...
export function startSiteImportJob({
  ...
  // Aus dem Feld kommt gern mehr als die Adresse – beim Einfügen landet
  // schon mal der ganze umgebende Text darin. Nur das erste Wort zählt, sonst
  // baut die Browser-URL-Klasse daraus eine 900 Zeichen lange Adresse mit
  // %20-Ketten und die Seite antwortet mit 404.
  const ersteAdresse = String(url || '').trim().split(/\s+/)[0] || '';
  let start;
  try {
    start = new URL(ersteAdresse);
  } catch {
      ...
      for (const hit of hits) {
        ...
        try {
          ...
          if (deps.findRecipeBySourceUrlPart(key) || deps.findRecipeByName(hit.recipe.name)) {
            ...
            // Die Kennung merken: die id im Spiegel gibt es erst nach dem
            // Abgleich weiter unten.
            angelegteKeys.push(key);
            job.imported += 1;
          } else {
            const angelegt = deps.createRecipe({ ...hit.recipe, source: 'web' });
            if (angelegt?.id) job.createdIds.push(angelegt.id);
            job.imported += 1;
          }
        } catch (err) {
          job.failed += 1;
