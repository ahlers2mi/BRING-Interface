...
// Wie viel Beleg eine Zutat braucht, um überhaupt ins Profil zu kommen.
// Gerechnet wird in **Gewicht**, nicht in Köpfen: zwei gekochte Bewertungen
// sind 2.0 und zählen, zwei bloß aussortierte Rezepte nur 1.2 und zählen
// nicht. Sonst brandmarkt ein einziges Aufräumen am Abend eine ganze Zutat
// („wir mögen keine Gnocchi", weil zwei Gnocchi-Rezepte rausgeflogen sind).
const MIN_EVIDENCE = 1.5;

function finalize(map, { minWeight = MIN_EVIDENCE } = {}) {
  return [...map.values()]
    .map((e) => ({
      key: e.key,
      label: e.label,
      ...
      confidence: e.weight / (e.weight + 2), // 1 Bewertung zählt wenig
    }))
...
function ownRatingFactor(recipe) {
  // Jedes "rausgeflogen" dämpft – und zwar unabhängig davon, ob das Rezept
  // je gekocht wurde. `rating_count` zählt nur gekochte Bewertungen mit
  // Sternen, ein bloß aussortiertes Rezept steht dort auf 0: der Abzug muss
  // deshalb VOR dem Neugier-Bonus greifen, sonst bleibt genau das Rezept
  // ungestraft, das man weggeklickt hat.
  const rejected = Number(recipe.rejected_count || 0);
  const damp = rejected > 0 ? Math.max(0.1, 0.25 ** rejected) : 1;

  const count = Number(recipe.rating_count || 0);
  // Noch nie gekocht: leichter Neugier-Bonus – aber nur, solange es nicht
  // schon einmal aussortiert wurde. Wer es weggeklickt hat, kennt es ja.
  if (!count) return (rejected > 0 ? 1 : 1.3) * damp;
