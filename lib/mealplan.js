...
import { climateBias, weatherFactor } from './climate.js';
...
function weightsFor(recipes, profile, { excludeIds, today, relax, date, bias }) {
  return recipes.map((recipe) => {
    ...
    return base * effortFactor(recipe, date) * weatherFactor(recipe, bias);
  });
}
...
export function rollRecipe({
  ...
  const bias = weather === undefined ? climateBias(date, currentWeather()) : weather;

  for (const relax of RELAXATIONS) {
    const weights = weightsFor(usable, profile, { excludeIds, today, relax, date, bias });
...
export function rollDays(dates, { overwrite = true, random = Math.random } = {}) {
  ...
  for (const date of dates) {
    ...
    const rolled = rollRecipe({ recipes, profile, excludeIds, today, date, random });
    if (!rolled) {
