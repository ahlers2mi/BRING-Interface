...
function weightsFor(recipes, profile, { excludeIds, today, relax, date }) {
  return recipes.map((recipe) => {
    ...
    return base * effortFactor(recipe, date);
  });
}
...
export function rollRecipe({
  ...
  excludeIds = new Set(),
  today = todayIso(),
  date = null, // für welchen Tag – entscheidet über den Aufwand
  random = Math.random,
}) {
  ...
  for (const relax of RELAXATIONS) {
    const weights = weightsFor(usable, profile, { excludeIds, today, relax, date });
