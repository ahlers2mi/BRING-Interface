...
export function rollDays(
  ...
  for (const date of dates) {
    ...
    // Dasselbe Gericht noch einmal auf denselben Tag ist kein Wurf – wer auf
    // den Würfel drückt, will etwas anderes. Nur wenn sonst gar nichts übrig
    // bleibt (winzige Rezeptsammlung, engem Zeitlimit), darf es wieder das
    // Gleiche werden.
    const excludeIds = current?.recipe_id
      ? new Set([...others, current.recipe_id])
      : others;
    const roll = (exclude) =>
      rollRecipe({
        recipes,
    ...
    let rolled = roll(excludeIds);
    if (!rolled && excludeIds !== others) rolled = roll(others);
    if (!rolled) {
...
// Fasst die Zutaten aller eingeplanten Rezepte zusammen.
//
// Zwei Dinge, die man beim Einkaufen sonst von Hand macht:
//
//   - **Gleiche Zutat, gleiche Einheit wird addiert** (`mergeAmounts`). Vorher
//     standen die Mengen bloß hintereinander: "½ TL + 1 TL + 1 TL Salz".
//   - **Was im Vorrat auf „da" steht, bleibt weg.** Der Grundstock (Salz,
//     Pfeffer, Öl …) muss nicht jede Woche mit auf die Liste. Nur `have` fällt
//     raus – „knapp" und „leer" gehören ausdrücklich drauf.
//
// Übersprungenes wird mitgemeldet und nicht stillschweigend geschluckt: die
// Zuordnung über `ingredientMatches` ist bewusst großzügig (Stämme + Komposita),
// da darf man sich nicht blind drauf verlassen.
export function weekShoppingItems(
  week,
  { skipCooked = true, servings = null, skipPantry = true } = {}
) {
  ...
  for (const day of view.days) {
    ...
    if (!recipe) continue;
    usedRecipes.push({ date: day.date, id: recipe.id, name: recipe.name });
    ...
    for (const ing of scaleIngredients(realIngredients(recipe.ingredients), factor)) {
      const imVorrat = vorhanden.find((name) => ingredientMatches(name, ing.name));
      if (imVorrat) {
        if (!pantrySkipped.some((p) => p.name === ing.name)) {
          pantrySkipped.push({ name: ing.name, pantry: imVorrat });
        }
        continue;
      }
  ...
  return { week, items, recipes: usedRecipes, pantrySkipped };
}
...
export function fridgeSearch(
  have,
  { assumePantry = true, limit = 20, includeBlocked = false, pantryNames = null } = {}
) {
  const haveList = have
    .map((h) => String(h || '').trim())
    .filter((h) => h.length >= 2);
  ...
  for (const recipe of recipes) {
    ...
    const match = matchRecipeToFridge(ingredients, haveList, { assumePantry, pantryNames });
