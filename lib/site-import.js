...
export async function verifyRecipeUrls(
  urls,
  { fetchImpl = fetch, delayMs = 250, limit = 50, log = () => {}, isCancelled = () => false }
) {
  const hits = [];
  let checked = 0;
  for (const url of urls) {
    ...
    try {
      ...
      if (recipe?.name) hits.push({ url, recipe });
    } catch (err) {
      log(`${url}: ${err.message}`);
    }
    await sleep(delayMs);
  }
  log(`${checked} Adressen geprüft, ${hits.length} davon sind Rezepte.`);
  return hits;
}
