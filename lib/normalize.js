...
export function splitIngredientText(text) {
  ...
  // 4. Eine Gewichtsangabe in Klammern am Ende gehört zur Menge, nicht zum
  //    Artikelnamen: aus "1 Dose/n Kokosmilch (ca. 400 g)" wird der Artikel
  //    "Kokosmilch" mit "1 Dose oder 400 g". Auf der Bring-Liste steht damit
  //    das, was man im Regal sucht, und daneben beides zum Vergleichen.
  const klammer = new RegExp(
    `^(.*?)\\s*\\(\\s*(?:ca\\.?|etwa|ungef(?:ä|ae)hr)?\\s*(\\d+(?:[.,]\\d+)?)\\s*(${UNITS.map(
      (u) => u.replace(/\./g, '\\.')
    ).join('|')})\\s*\\)$`,
    'i'
  );
  ...
  if (inKlammern && inKlammern[1].trim()) {
    const menge = `${formatAmountNumber(Number(inKlammern[2].replace(',', '.'))) || inKlammern[2]} ${inKlammern[3]}`;
    amount = amount ? `${amount} oder ${menge}` : menge;
