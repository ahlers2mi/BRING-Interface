...
const USER_AGENT =
  ...
  'BRING-Interface/1.0 (privater Haushalt; Rezeptimport)';
...
export function extractChefkochIdsFromApi(json) {
  ...
  for (const item of results) {
    ...
    if (recipe?.id) ids.push(String(recipe.id));
  }
  return ids;
}
...
export async function fetchRecipeFromUrl(url, { fetchImpl = fetch } = {}) {
  const ckId = /chefkoch\.de\/rezepte\/(\d{6,})/.exec(url)?.[1];
  if (ckId) {
    try {
...
export async function discoverChefkochIds({
  query = '',
  count = 50,
  fetchImpl = fetch,
  log = () => {},
  ...
  for (let offset = 0; ids.length < wanted && offset < wanted + 200; offset += 50) {
    ...
    try {
      ...
      const found = extractChefkochIdsFromApi(json);
...
export function startImportJob({
  ...
  (async () => {
    try {
      ...
      const ids = await discoverChefkochIds({
        query,
        count: job.requested,
        fetchImpl,
        log,
