...
// Schwellen: Vorgabe aus der Umgebung, überschreibbar je Aufruf (die
// Einstellungen der App reichen sie durch).
export const DEFAULT_COLD_C = Number(process.env.PLAN_COLD_C || 10);
export const DEFAULT_WARM_C = Number(process.env.PLAN_WARM_C || 24);
...
export function climateBias(
  date,
  {
    temp = null,
    measuredAt = null,
    now = null,
    coldC = DEFAULT_COLD_C,
    warmC = DEFAULT_WARM_C,
  } = {}
) {
  ...
  if (Number.isFinite(value) && measuredAt && abstand >= 0 && abstand <= 1) {
    ...
    if (Number.isFinite(alter) && alter >= 0 && alter <= FRESH_HOURS) {
      if (value <= coldC) return 'kalt';
      if (value >= warmC) return 'warm';
      return null;
    }
  }
