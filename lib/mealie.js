...
export function looksThin(recipe) {
  const ingredients = (recipe?.recipeIngredient || []).length;
...
// Rückgabe: { outcome: 'repaired' | 'not-needed' | 'no-data', detail? }
...
export async function repairThinMealieRecipe(
  ...
  if (!looksThin(inMealie)) return { outcome: 'not-needed' };

  const fromApi = await fetchChefkochRecipe(id, { fetchImpl });
  if (!fromApi) return { outcome: 'no-data' };
  ...
  if (!steps.length && !betterIngredients) return { outcome: 'no-data' };
