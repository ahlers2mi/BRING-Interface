...
import { discoverChefkochIds } from './recipe-import.js';
...
export function startChefkochToMealieJob({ query = '', count = 20, deps, fetchImpl = fetch }) {
  if (chefkochJob && chefkochJob.status === 'running') {
  ...
  (async () => {
    try {
      log(`Suche ${job.requested} Rezepte auf Chefkoch${query ? ` zu "${query}"` : ''} …`);
